...
" [CTRL + F3 插入(年月日时分秒)]
nmap <C-F3> a<C-R>=strftime("%Y-%m-%d %T")<CR><Esc>
imap <C-F3> <C-R>=strftime("%Y-%m-%d %T")<CR>
...
" [快速打开自定义手册]
if has("win32")
    nnoremap <leader>man :split $HOME/vimfiles/manual.txt<cr>
else
    nnoremap <leader>man :split $HOME/.vim/manual.txt<cr>
endif
